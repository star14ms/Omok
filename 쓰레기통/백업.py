        ...
        if x_r > -1 and x_r+3 < size and y_d > -1 and y_d+3 < size:
            ...
            if sum(line) == whose_turn*3:
                if (x_r-1 > -1) and (y_d-1 > -1) and (x_r+4 < size) and (y_d+4 < size):
                    ...
                    if (board[y_d-1][x_r-1] == 0) and (board[y_d+4][x_r+4] == 0): ### x_r -> x_r-1, x_r -> x_r+4
...
def difference_score_board(whose_turn, size, board):
    ...
    for y in range(size):
        for x in range(size):
            # if oneself_score_board[y][x] >= opponent_score_board[y][x]:
            #     total_score_board[y][x] = oneself_score_board[y][x]
            # else:
            #     total_score_board[y][x] = opponent_score_board[y][x]
